...
public class SolicitudPagoDAO {
	...
	public List<OficioObsSolicitudPago> consultaOficioObsSolicitudPago(long idOficioObsSolPago) throws JDBCException {
		return consultaOficioObsSolicitudPago(null, idOficioObsSolPago, null); 
	}
	
	@SuppressWarnings("unchecked")
	public List<OficioObsSolicitudPago> consultaOficioObsSolicitudPago(String folioCartaAdhesion, long idOficioObsSolPago, Boolean alcance) throws JDBCException {
		StringBuilder consulta= new StringBuilder();
	...
	public List<Programa> consultarProgramasPrimerPagoEfectuado(){
		...
		consulta.append("select p.id_programa, p.descripcion_corta,p.descripcion_larga, p.ciclo, p.ciclo_corto, p.producto, p.estados, p.descripcion, p.ruta_documentos, p.id_area, p.criterio_pago, p.id_componente,	p.id_unidad_medida,	p.numero_etapa,	p.des_apoyo_en_oficio_clc ")
				...
				.append("where pa.id_programa = p.id_programa ")
				.append("and pa.id_pago in (select id_pago ")
											.append("from pagos pg ")
											.append("where pg.fecha_pago = (select min(fecha_pago) ")
																		.append("from pagos ")
																		.append("where estatus = 5 ")
																		.append("and no_carta = pa.no_carta ")
																		.append("and id_programa = pa.id_programa) ")
											.append("and pg.estatus = 5 ")
											.append("and pg.no_carta = pa.no_carta ")
											.append("and pg.id_programa = pa.id_programa) ")
	...
	@SuppressWarnings("unchecked")
	public List<CartasPrimerPagoV> consultarCartasPrimerPagoEfectuado(int idPrograma, String folioCartaAdhesion){
		List<CartasPrimerPagoV> lst = null;
		StringBuilder consulta= new StringBuilder();
		...
		consulta.insert(0, "From CartasPrimerPagoV ");
		lst= session.createQuery(consulta.toString()).list();
	...
	@SuppressWarnings("unchecked")
	public List<Expediente> consultaCAExpedienteFianza(String folioCartaAdhesion, int idPrograma) throws JDBCException {
		List<Expediente> lst=null;
		StringBuilder consulta= new StringBuilder();
		consulta.append("select e.id_expediente, e.expediente, e.descripcion, e.tipo, e.prioridad_expediente " +
				"from expedientes_programas  ep, expedientes e where  not exists (select * from documentacion_sp_carta_adhesion  d " +
				"where d.id_expediente = ep.id_expediente and d.folio_carta_adhesion = " +
				"'").append(folioCartaAdhesion).append("') ").append(" and ep.id_programa = ").append(idPrograma)
				.append(" and ep.documentacion_opcional=false and ep.id_expediente = e.id_expediente and e.tipo in ('F','DSPYF')").append(" order by e.prioridad_expediente");
