...
public class InicializacionProgramaAction extends ActionSupport implements SessionAware{
	...
	private List<Cultivo> lstCulVolXCultivoVariedad;
	private List<Variedad> lstVarVolXCultivoVariedad;
	...
	public String listarProgramas(){
		try{
			...
			if(idPerfil == 3 || idPerfil == 10 || idPerfil == 4){
				lstProgramaV = cDAO.consultaProgramaV(idPrograma);	
			}else if(idPerfil == 5){
	...
	public String agregaCultivoEstadoIniProg(){
		...
		if(editar == 0 || editar == 1){
			...
			if(idCriterioPago == 1 || idCriterioPago == 3){
				if(volxCulVar == 0){
					lstCultivo = iDAO.recuperaCultivoByVolumenXCultivoVariedad(idPrograma);
					lstVariedad = iDAO.recuperaVariedadByVolumenXCultivoVariedad(idPrograma);
				}else{
					lstCultivo = cDAO.consultaCultivo();
	...
	public String agregarVolumenPorCultivoVariedad(){
		try{
			...
			if(editar == 0){
				...
				lstCulVolXCultivoVariedad = cDAO.consultaCultivo();
				lstVarVolXCultivoVariedad = new ArrayList<Variedad>();
			}else{
				lstCultivoVariedadEsquema = iDAO.consultaCultivoVariedadEsquema(idPrograma);
				lstCulVolXCultivoVariedad = cDAO.consultaCultivo();
				lstVarVolXCultivoVariedad =  iDAO.recuperaVariedadByVolumenXCultivoVariedad(idPrograma);
			}
...
//			lstVariedad = new ArrayList<Variedad>();
	...
	private void integrarCiclo() {
		...
		if(sizeArrayCiclo>1){
			...
			if(selecCiclo[0]==1){
				...
				anio = Integer.parseInt(selecAnio[0].toString().substring(2,4));
				programa.setCiclo("Otoño-Invierno"+" "+selecAnio[0]+"/"+(selecAnio[0].intValue()+1));
				programa.setCicloCorto("OI-"+anio+"/"+(anio+1));
			}
		}	
	...
	public int getNumCamposVXCV() {
		return numCamposVXCV;
	}
	
	public void setNumCamposVXCV(int numCamposVXCV) {
	...
	public List<Cultivo> getLstCulVolXCultivoVariedad() {
		return lstCulVolXCultivoVariedad;
	}
	public void setLstCulVolXCultivoVariedad(List<Cultivo> lstCulVolXCultivoVariedad) {
		this.lstCulVolXCultivoVariedad = lstCulVolXCultivoVariedad;
	}
	
	public List<Variedad> getLstVarVolXCultivoVariedad() {
		return lstVarVolXCultivoVariedad;
	}
