...
public class RelacionComprasAction extends ActionSupport implements SessionAware, ServletContextAware {
	...
	private  String criteriosByPrograma;
			   ...
			   if(contColumna == lstGruposCamposDetalleRelacionV.size()){	//Penultima columna que lee la bodega
				   if(contRow >= lstGruposCamposEncabezadoRelacionV.size()){
					   if(valor==null || valor.isEmpty()){
						   msj = "Fila: "+(contRow+1)+". Valor nulo en bodega "+valor;
...
//							}
				   ...
				   if(contRow >= lstGruposCamposEncabezadoRelacionV.size()){
					   if(valor==null || valor.isEmpty()){
						   bd = new BitacoraRelcomprasDetalle();
						   msj = "Fila: "+contRow+". Valor nulo en estado "+valor;
						   bd.setMensaje(msj);
						   b.getBitacoraRelcomprasDetalle().add(bd);
						   errorInternoArchivo = true;
						   errorLecturaArchivo = true;
...
//					  } 
					  ...
					  if(valor!=null && !valor.isEmpty()){
						  if(verificarTipoDato(valor, lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato())){
							  AppLogger.info("app","fila: "+contRow+" Columna"+contColumna+" grupo 6 campo 69 el valor es "+valor);
							  if(valor.length() >= 1 && valor.length() <= 18){
								  rcTmp.setCurpProductor(valor);
								 }else{
			   ...
			   if(lstGruposCamposDetalleRelacionV.get(contColumna).getIdCampo().intValue() == 23){//"PAGO DEL PRECIO PACTADO EN AXC POR TONELADA ($)"
				   if(valor!=null && !valor.isEmpty()){
					  if(verificarTipoDato(valor, lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato())){
						  AppLogger.info("app","fila "+contRow+" Columna"+contColumna+" grupo 11 campo 23 el valor es "+valor);
						  //rcTmp.setImp(Double.parseDouble(valor));
					  }else{
						  AppLogger.info("app", lstGruposCamposDetalleRelacionV.get(contColumna).getCampo()+" Fila :"+(contRow+1)+" Columna: "+(contColumna+1)+". Valor no permitido, se esperaba valor de tipo "+ lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato());								  
					  } 								  		 
				   }
			   }else if(lstGruposCamposDetalleRelacionV.get(contColumna).getIdCampo().intValue() == 24){ //"PAGO DEL PRECIO PACTADO EN AXC IMPORTE ($)"
				   if(valor!=null && !valor.isEmpty()){
					  if(verificarTipoDato(valor, lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato())){
						  AppLogger.info("app","fila "+contRow+" Columna"+contColumna+" grupo 11 campo 26 el valor es "+valor);
						  //rcTmp.setImpCompTonTotalAXC(Double.parseDouble(valor));
					  }else{
						  AppLogger.info("app", lstGruposCamposDetalleRelacionV.get(contColumna).getCampo()+" Fila :"+(contRow+1)+" Columna: "+(contColumna+1)+". Valor no permitido, se esperaba valor de tipo "+ lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato());								  
					  } 								  		 
				   }
			   }else if(lstGruposCamposDetalleRelacionV.get(contColumna).getIdCampo().intValue() == 25){ //" IMPORTE DE LA COMPENSACIÓN POR TON. ($)"
				   if(valor!=null && !valor.isEmpty()){
					  if(verificarTipoDato(valor, lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato())){
						  AppLogger.info("app","fila "+contRow+" Columna"+contColumna+" grupo 11 campo 26 el valor es "+valor);
						  rcTmp.setImpCompTonAXC(Double.parseDouble(valor));
					  }else{
						  AppLogger.info("app", lstGruposCamposDetalleRelacionV.get(contColumna).getCampo()+" Fila :"+(contRow+1)+" Columna: "+(contColumna+1)+". Valor no permitido, se esperaba valor de tipo "+ lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato());								  
					  } 								  		 
				   }
			   }else if(lstGruposCamposDetalleRelacionV.get(contColumna).getIdCampo().intValue() == 26){ //"IMPORTE TOTAL DE LA COMPENSACIÓN ($)"
				   if(valor!=null && !valor.isEmpty()){
					  if(verificarTipoDato(valor, lstGruposCamposDetalleRelacionV.get(contColumna).getTipoDato())){
						  AppLogger.info("app","fila "+contRow+" Columna"+contColumna+" grupo 11 campo 26 el valor es "+valor);
						  rcTmp.setImpCompTonTotalAXC(Double.parseDouble(valor));
...
//			}			
	...
	private void guardarRelacioncompras(int numeroHoja){
			...
			if(numRegNoGuardados > 0){
				msj = "No fue posible guardar "+numRegNoGuardados+(numRegNoGuardados == 1?" registro":" registros")+" en la base de datos de la hoja"+numeroHoja+", por favor verifique";
				lstLog.add(msj);
			}	
	...
	public String recuperaCriterioAplicarPeriodo(){
		if(pideTipoPeriodo == 3){
			...
			List<AuditorSolicitudPagoV> lstAuditorSolPagoV = spDAO.consultaAuditorSolPagoV(folioCartaAdhesion, -1, -1);
			if(lstAuditorSolPagoV.size() > 0){
				fechaInicioAuditor = lstAuditorSolPagoV.get(0).getFechaInicio();
				fechaFinAuditor = lstAuditorSolPagoV.get(0).getFechaFin();
			}
...
//			}
	...
	public String prevalidarRelacionCompras(){
		...
		String fcProductor = "", fcTmpProductor = ""; 
		...
		try{
			...
			idPerRel =  lstPersonalizacionRelacionesProgramaV.get(0).getIdPerRel();		
			lstValidacionPorGrupo = rDAO.recuperaCatCriteriosValidacionByGurpoPrograma(idPerRel,null,null);
			for(CatCriteriosValidacion l: lstValidacionPorGrupo){
				criteriosByPrograma += l.getIdCriterio()+",";
			}
			...
			if(grupoCriterio.equals("Predio")){
				...
				for(CatCriteriosValidacion l:lstValidacionPorGrupo){
					if(l.getIdCriterio()== 1){//PREDIO EXISTA EN BASE DE DATOS
						sheet = wb.createSheet("PRED EXISTA EN BD"); //"7.4 DIFERENCIAS PREDIOS/PRODUCTOR/COMPRADOR RESPECTO A BASE DE DATOS"
...
//							cell = row.createCell(++countColumn);
							...
							for(RelacionComprasTMP p: lstPrediosNoExistenBD){
								...
								bd.setMensaje(p.getClaveBodega()+";"+p.getNombreEstado()+";"+p.getPaternoProductor()+";"
										...
										+p.getCurpProductor()+";"+p.getRfcProductor()+";"
										+p.getFolioPredio()+";"
										+p.getBancoSinaxc());
								b.getBitacoraRelcomprasDetalle().add(bd);
...
//								cell = row.createCell(++countColumn);
								...
								cell = row.createCell(++countColumn);
								cell.setCellValue(p.getBancoSinaxc());
							}							
							b.setMensaje(msj);
							cDAO.guardaObjeto(b);
						}else{
				...
				for(CatCriteriosValidacion l:lstValidacionPorGrupo){
					if(l.getIdCriterio()== 7){//7.2 FALTA DE INFORMACIÓN DE AL MENOS UN REGISTRO EN BOLETA, FACTURA, PREDIO Y PAGO
						...
						if(lstRfcProductorVsRfcFactura.size()>0){//En el listado se guardan las facturas duplicadas por productor
							...
							for(RfcProductorVsRfcFactura r: lstRfcProductorVsRfcFactura){
								...
								rDAO.actualizaFacMayBolOPagMenFac(folioCartaAdhesion, r.getClaveBodega(), r.getNombreEstado(), r.getFolioContrato(),
										r.getPaternoProductor(), r.getMaternoProductor(), r.getNombreProductor(), r.getCurpProductor(), r.getRfcProductor(), false, false, true, false);	
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
					...
					/*}else if(l.getIdCriterio() == 31){
						...
						if(lstBoletasFacturasPagosIncosistentes.size()>0){//En el listado se guardan las boletas, facturas y pagos incosistentes por productores
							...
							for(BoletasFacturasPagosIncosistentes bod: lstBoletasFacturasPagosIncosistentes){
								...
								bd.setMensaje(bod.getClaveBodega()+";"+bod.getNombreEstado()+";"+(siAplicaFolioContrato?bod.getFolioContrato()+";":"")+bod.getPaternoProductor()+";"+bod.getMaternoProductor()+";"
										...
										+bod.getCurpProductor()+";"+bod.getRfcProductor()+";"
										+bod.getVolTotalFacturado()+";"+bod.getVolBolTicket()+";"+bod.getVolTotalFacVenta()+";"+bod.getVolEnpagos()+";"
								...
								cell = row.createCell(++countColumn);
								cell.setCellValue(bod.getCurpProductor()!=null ?bod.getCurpProductor():bod.getRfcProductor());
								cell = row.createCell(++countColumn);
								...
								if(criteriosByPrograma.contains(",1,")){ //Precio no pagado
									cell = row.createCell(++countColumn);
									cell.setCellValue(bod.getPredioNoPagado()!=null?bod.getPredioNoPagado()+"":"");										
								}
								if(criteriosByPrograma.contains(",19,")){ //Diferencia entre factura global e individual
									cell = row.createCell(++countColumn);
						...
						if(lstRendimientosProcedente.size()>0){//En el listado se guardan los volumenes no procedentes
							...
							for(RendimientosProcedente bod: lstRendimientosProcedente){
								...
								bd = new BitacoraRelcomprasDetalle();
								bd.setMensaje(bod.getClaveBodega()+";"+bod.getNombreEstado()+";"+(siAplicaFolioContrato?bod.getFolioContrato()+";":"")+bod.getPaternoProductor()+";"+bod.getMaternoProductor()+";"
										+bod.getNombreProductor()+";"+bod.getCurpProductor()+";"+bod.getRfcProductor()+";"
										+bod.getVolTotalFacturado()+";"+bod.getRendimientoMaximoAceptable()+";"
								...
								rDAO.actualizaVolumenNoProcedente(folioCartaAdhesion, bod.getClaveBodega(), bod.getNombreEstado(), bod.getFolioContrato(),
										bod.getPaternoProductor(), bod.getMaternoProductor(), bod.getNombreProductor(), bod.getCurpProductor(), bod.getRfcProductor(), bod.getVolNoProcedente());	// AHS [LINEA] - 17022015
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(bod.getClaveBodega()!=null ? bod.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
						...
						if(lstVolNoProcedenteYApoyEnReg.size()>0){//En el listado se guardan los volumenes no procedentes
							...
							for(VolumenNoProcedente bod: lstVolNoProcedenteYApoyEnReg){
								...
								bd.setMensaje(bod.getClaveBodega()+";"+bod.getNombreEstado()+";"+(siAplicaFolioContrato?bod.getFolioContrato()+";":"")+bod.getPaternoProductor()+";"+bod.getMaternoProductor()+";"
										+bod.getNombreProductor()+";"+bod.getCurpProductor()+";"+bod.getRfcProductor()+";"
										+bod.getIdVariedad()+";"+bod.getVolTotalFacturado()+";"+bod.getVolApoyadoEnRegional()+";"
										+(bod.getVolNoProcedente()!=null?bod.getVolNoProcedente():"0"));  
								b.getBitacoraRelcomprasDetalle().add(bd);		
								//Actualiza el productor como inconsistente
								rDAO.actualizaVolumenNoProcedente(folioCartaAdhesion, bod.getClaveBodega(), bod.getNombreEstado(), bod.getFolioContrato(),
										bod.getPaternoProductor(), bod.getMaternoProductor(), bod.getNombreProductor(), bod.getCurpProductor(), bod.getRfcProductor(), bod.getVolNoProcedenteTotal().toString());	// AHS [LINEA] - 17022015
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(bod.getClaveBodega()!=null ? bod.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
					...
					if(l.getIdCriterio() == 8){//"BOLETAS NO ESTEN DUPLICADAS POR BODEGA"
						...
						if(lstBoletasDuplicadas.size()>0){//En el listado se guardan las boletas duplicadas por bodega
							...
							for(BoletasDuplicadas bod: lstBoletasDuplicadas){
								...
								bd.setMensaje(bod.getClaveBodega()+";"+bod.getNombreEstado()+";"+(siAplicaFolioContrato?bod.getFolioContrato()+";":"")+bod.getPaternoProductor()+";"+bod.getMaternoProductor()+";"
										+bod.getNombreProductor()+";"+bod.getCurpProductor()+";"+bod.getRfcProductor()+";"
										+bod.getBoletaTicketBascula()+";"+bod.getVolBolTicket()+";"+bod.getFechaEntradaBoleta());
								b.getBitacoraRelcomprasDetalle().add(bd);
								actualizarRelacionComprasTMPByBoletasIncosistentes(folioCartaAdhesion, bod.getClaveBodega(),bod.getNombreEstado(),bod.getFolioContrato(), 
										bod.getPaternoProductor(), bod.getMaternoProductor(), bod.getNombreProductor(),bod.getCurpProductor(),bod.getRfcProductor(), bod.getBoletaTicketBascula());
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(bod.getClaveBodega()!=null ? bod.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
							...
							if(lstBoletasFueraDePeriodo.size()>0){//En el listado se guardan las boletas que estan fuera del periodo seleccionado
								...
								for(BoletasFueraDePeriodo bfp: lstBoletasFueraDePeriodo){
									...
									bd.setMensaje(bfp.getClaveBodega()+";"+bfp.getNombreEstado()+";"+(siAplicaFolioContrato?bfp.getFolioContrato()+";":"")
												...
												+bfp.getCurpProductor()+";"+bfp.getRfcProductor()+";"
									...
									actualizarRelacionComprasTMPByBoletasIncosistentes(folioCartaAdhesion, bfp.getClaveBodega(),bfp.getNombreEstado(),bfp.getFolioContrato(), 
											bfp.getPaternoProductor(), bfp.getMaternoProductor(), bfp.getNombreProductor(),bfp.getCurpProductor(),bfp.getRfcProductor(), bfp.getBoletaTicketBascula());
									row = sheet.createRow(++countRow);
									cell = row.createCell(countColumn);
									...
									cell = row.createCell(++countColumn);
									cell.setCellValue(bfp.getCurpProductor()!=null ?bfp.getCurpProductor():bfp.getRfcProductor());
									cell = row.createCell(++countColumn);
					    ...
					    if(lstBoletasCamposRequeridos.size()>0){
							...
							for(BoletasCamposRequeridos bo: lstBoletasCamposRequeridos){
								...
								bd.setMensaje(bo.getClaveBodega()+";"+bo.getNombreEstado()+";"+(siAplicaFolioContrato?bo.getFolioContrato()+";":"")
										+bo.getPaternoProductor()+";"+bo.getMaternoProductor()+";"+bo.getNombreProductor()+";"
										+bo.getCurpProductor()+";"+bo.getRfcProductor()+";"
										+bo.getBoletaTicketBascula()+";"
								...
								actualizarRelacionComprasTMPByBoletasIncosistentes(folioCartaAdhesion, bo.getClaveBodega(),bo.getNombreEstado(),bo.getFolioContrato(), 
										bo.getPaternoProductor(), bo.getMaternoProductor(), bo.getNombreProductor(), bo.getCurpProductor(), bo.getRfcProductor(), bo.getBoletaTicketBascula());
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								...
								cell = row.createCell(++countColumn);
								cell.setCellValue(bo.getCurpProductor()!=null ?bo.getCurpProductor():bo.getRfcProductor());
								cell = row.createCell(++countColumn);
					...
					if(lstProductorFactura.size()>0){//En el listado se guardan las facturas duplicadas por productor
						...
						for(ProductorFactura pf: lstProductorFactura){
							...
							bd.setMensaje(pf.getClaveBodega()+";"+pf.getNombreEstado()+";"+(siAplicaFolioContrato ? pf.getFolioContrato()+";":"")+
									pf.getPaternoProductor()+";"+pf.getMaternoProductor()+";"+pf.getNombreProductor()+";"
									+pf.getCurpProductor()+";"+pf.getRfcProductor()+";"+pf.getFolioFacturaVenta()+";"
							...
							actualizarRelacionComprasTMPByFacturasIncosistentes(folioCartaAdhesion, pf.getClaveBodega(),pf.getNombreEstado(),pf.getFolioContrato(), 
									pf.getPaternoProductor(), pf.getMaternoProductor(), pf.getNombreProductor(),pf.getCurpProductor(), pf.getRfcProductor(),  pf.getFolioFacturaVenta());
							row = sheet.createRow(++countRow);
							cell = row.createCell(countColumn);
							...
							cell = row.createCell(++countColumn);
							cell.setCellValue(pf.getCurpProductor()!=null ?pf.getCurpProductor():pf.getRfcProductor());
							cell = row.createCell(++countColumn);
						...
						if(lstBoletasVsFacturas.size()>0){//VOLUMEN FACTURADO MAYOR
							...
							for(BoletasVsFacturas bf: lstBoletasVsFacturas){
								...
								bd.setMensaje(bf.getClaveBodega()+";"+bf.getNombreEstado()+";"+(siAplicaFolioContrato?bf.getFolioContrato()+";":"")+
										...
										+bf.getCurpProductor()+";"+bf.getRfcProductor()+";"
								...
								rDAO.actualizaFacMayBolOPagMenFac(folioCartaAdhesion, bf.getClaveBodega(), bf.getNombreEstado(), bf.getFolioContrato(),
										bf.getPaternoProductor(), bf.getMaternoProductor(), bf.getNombreProductor(), bf.getCurpProductor(), bf.getRfcProductor(), true, false);
								...
								rDAO.actualizaDiferenciaVolumenXFacturaMayor(folioCartaAdhesion, bf.getClaveBodega(), bf.getNombreEstado(), bf.getFolioContrato(),
										bf.getPaternoProductor(), bf.getMaternoProductor(), bf.getNombreProductor(), bf.getCurpProductor(), bf.getRfcProductor(), difVolumenFacMayorBoleta);	
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								...
								cell = row.createCell(++countColumn);
								cell.setCellValue(bf.getCurpProductor()!=null ?bf.getCurpProductor():bf.getRfcProductor());								
								cell = row.createCell(++countColumn);
							...
							if(lstFacturaFueraPeriodo.size()>0){//En el listado se guardan las facturas duplicadas por productor
								...
								for(FacturaFueraPeriodoAuditor f: lstFacturaFueraPeriodo){
									...
									bd.setMensaje(f.getClaveBodega() + ";"+f.getNombreEstado()+";"+(siAplicaFolioContrato?f.getFolioContrato()+";":"")
											...
											+ f.getCurpProductor()+";"+f.getRfcProductor()+";"
									...
									actualizarRelacionComprasTMPByFacturasIncosistentes(folioCartaAdhesion, f.getClaveBodega(),f.getNombreEstado(),f.getFolioContrato(), 
											f.getPaternoProductor(), f.getMaternoProductor(), f.getNombreProductor(), f.getCurpProductor(),f.getRfcProductor(), f.getFolioFacturaVenta());
									row = sheet.createRow(++countRow);
									cell = row.createCell(countColumn);
									...
									cell = row.createCell(++countColumn);
									cell.setCellValue(f.getCurpProductor() != null ?f.getCurpProductor():f.getRfcProductor());
									cell = row.createCell(++countColumn);
							...
							if(lstFacturaFueraPeriodoPago.size()>0){//En el listado se guardan las facturas que estan fuera del periodo seleccionado
								...
								for(FacturaFueraPeriodoPago bfp: lstFacturaFueraPeriodoPago){
									...
									actualizarRelacionComprasTMPByFacturasIncosistentes(folioCartaAdhesion, bfp.getClaveBodega(),bfp.getNombreEstado(),bfp.getFolioContrato(), 
											bfp.getPaternoProductor(), bfp.getMaternoProductor(), bfp.getNombreProductor(), bfp.getCurpProductor(), bfp.getRfcProductor(), bfp.getFolioFacturaVenta());
									row = sheet.createRow(++countRow);
									cell = row.createCell(countColumn);
									cell.setCellValue(bfp.getClaveBodega()!=null ? bfp.getClaveBodega()+"":"");
									cell = row.createCell(++countColumn);
						...
						if(lstPrecioPagPorTipoCambio.size()>0){
							...
							cell = row.createCell(++countColumn);
							cell.setCellValue("Precio Fac MXP X TON");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Total Pag Factura en Dic Audit");
							cell = row.createCell(++countColumn);
							...
							cell = row.createCell(++countColumn);
							cell.setCellValue("Dif Monto Total");
							for(PrecioPagPorTipoCambio c: lstPrecioPagPorTipoCambio){
								countColumn = 0;
								fcProductor = c.getFolioContrato()+(c.getCurpProductor() != null?c.getCurpProductor():c.getRfcProductor());
								bd = new BitacoraRelcomprasDetalle();								
								bd.setMensaje( c.getClaveBodega()+";"+c.getNombreEstado()+";"+(siAplicaFolioContrato ? c.getFolioContrato()+";":"")
										...
										+c.getCurpProductor()+";"+c.getRfcProductor()+";"
										+c.getFolioFacturaVenta()+";"
										+(c.getVolTotalFacVenta()!=null?c.getVolTotalFacVenta()+";":";0")
										+(c.getFechaEmisionFac()!=null?c.getFechaEmisionFac()+";":";0")
										+(c.getPrecioFacMxpTon()!=null?c.getPrecioFacMxpTon()+";":";0")										
										+(c.getImpSolFacVenta()!=null?c.getImpSolFacVenta()+";":";0")
										+(c.getPrecioPactadoPorTonelada()!=null?c.getPrecioPactadoPorTonelada()+";":";0")
										+(c.getTipoCambio()!=null?c.getTipoCambio()+";":";0")
										+(c.getImporteCalculadoPagar()!=null?c.getImporteCalculadoPagar()+";":";0")
										+(c.getDifMontoXFac()!=null?c.getDifMontoXFac()+";":";0")
										+(c.getDifMontoTotal()!=null?c.getDifMontoTotal()+";":";0"));
								b.getBitacoraRelcomprasDetalle().add(bd);								
								...
								cell = row.createCell(++countColumn);
//								cell.setCellValue(c.getCurpProductor()!=null ?c.getCurpProductor():c.getRfcProductor());
								cell.setCellValue(c.getFolioFacturaVenta());
								cell = row.createCell(++countColumn);
								cell.setCellValue(c.getVolTotalFacVenta()!=null?c.getVolTotalFacVenta()+"":"");							
								cell = row.createCell(++countColumn);
								cell.setCellValue(c.getFechaEmisionFac()!=null?c.getFechaEmisionFac()+"":"");								
								cell = row.createCell(++countColumn);
								cell.setCellValue(c.getPrecioFacMxpTon()!=null?c.getPrecioFacMxpTon()+"":"");								
								cell = row.createCell(++countColumn);
								...
								cell = row.createCell(++countColumn);
								cell.setCellValue(c.getDifMontoXFac()!=null?c.getDifMontoXFac()+"":"");
								cell = row.createCell(++countColumn);
								if(!fcTmpProductor.equals(fcProductor)){
									cell.setCellValue(c.getDifMontoTotal()!=null?c.getDifMontoTotal()+"":"");
								}else{
									cell.setCellValue("");
								}
								
								fcTmpProductor = c.getFolioContrato()+(c.getCurpProductor() != null?c.getCurpProductor():c.getRfcProductor());
					...
					}else if(l.getIdCriterio() == 19){//""
						sheet = wb.createSheet("DIFERENCIA FAC GLOB VS FAC IND");
						sheet = setMargenSheet(sheet);
						countRow = 0;
						countColumn = 0;
						lstFGlobalVsFIndividual = rDAO.consultaFacGlobalVsFacIndividual(folioCartaAdhesion); 
						if(lstFGlobalVsFIndividual.size() > 0){//"" 
							llenarBitacora(true, l.getIdCriterio()); //Guardar en bitacora
							msj = l.getCriterio();
							row = sheet.createRow(countRow);
							cell = row.createCell(countColumn);
							...
							cell.setCellValue("CURP/RFC");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Factura Global");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Peso Neto Fac Global");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Peso Neto Fac Individual");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Diferencia Volumen");
							cell = row.createCell(++countColumn);																					
							for(RelacionComprasTMP f: lstFGlobalVsFIndividual){
								countColumn = 0;
								bd = new BitacoraRelcomprasDetalle();
								bd.setMensaje(f.getClaveBodega()+";"+f.getNombreEstado()+";"+(siAplicaFolioContrato?f.getFolioContrato()+";":"")+
										f.getPaternoProductor()+";"+f.getMaternoProductor()+";"+f.getNombreProductor()+";"+
										f.getCurpProductor()+";"+f.getRfcProductor()+";"+
										f.getNumeroFacGlobal()+";"+
										f.getVolPnaFacGlobal()+";"+
										(f.getVolTotalFacVenta()!=null?f.getVolTotalFacVenta()+";":"0;")+
										(f.getDifVolumenFglobalVSFind()!=null?f.getDifVolumenFglobalVSFind():0));
								b.getBitacoraRelcomprasDetalle().add(bd);								
								...
								cell.setCellValue(f.getCurpProductor()!=null ?f.getCurpProductor():f.getRfcProductor());
								cell = row.createCell(++countColumn);
								cell.setCellValue(f.getNumeroFacGlobal());
								cell = row.createCell(++countColumn);
								cell.setCellValue(f.getVolPnaFacGlobal()!=null?f.getVolPnaFacGlobal()+"":"");
								cell = row.createCell(++countColumn);
								cell.setCellValue(f.getVolTotalFacVenta()!=null?f.getVolTotalFacVenta()+"":"");
								cell = row.createCell(++countColumn);
								cell.setCellValue(f.getDifVolumenFglobalVSFind()!=null?f.getDifVolumenFglobalVSFind()+"":"");
							}							
							b.setMensaje(msj);
							cDAO.guardaObjeto(b);
						}else{ 
						...
						if(lstFacturasCamposRequeridos.size()>0){
							...
							for(FacturasCamposRequeridos bo: lstFacturasCamposRequeridos){
								...
								bd.setMensaje(bo.getClaveBodega()+";"+bo.getNombreEstado()+";"+(siAplicaFolioContrato?bo.getFolioContrato()+";":"")
										+bo.getPaternoProductor()+";"+bo.getMaternoProductor()+";"+bo.getNombreProductor()+";"
										+bo.getCurpProductor()+";"+bo.getRfcProductor()+";"+bo.getFolioFacturaVenta()+";"										
								...
								actualizarRelacionComprasTMPByFacturasIncosistentes(folioCartaAdhesion, bo.getClaveBodega(),bo.getNombreEstado(),bo.getFolioContrato(), 
										bo.getPaternoProductor(), bo.getMaternoProductor(), bo.getNombreProductor(), bo.getCurpProductor(), bo.getRfcProductor(), bo.getFolioFacturaVenta());
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(bo.getClaveBodega()!=null ? bo.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
					...
					if(l.getIdCriterio() == 16){//"NO SE REPITAN CHEQUES-BANCO POR EMPRESA"
						...
						if(lstChequesDuplicadoBancoPartipante.size()>0){//En el listado se guarda los cheque duplicados por banco empresa
							...
							for(ChequesDuplicadoBancoPartipante c: lstChequesDuplicadoBancoPartipante){
								...
								bd.setMensaje( c.getClaveBodega()+";"+c.getNombreEstado()+";"+(siAplicaFolioContrato ? c.getFolioContrato()+";":"")
										+c.getPaternoProductor()+";"+c.getMaternoProductor()+";"+c.getNombreProductor()+";"
										+c.getCurpProductor()+";"+c.getRfcProductor()+";"+c.getFolioDocPago() + ";"
								...
								actualizarRelacionComprasTMPByPagosDuplicados(folioCartaAdhesion, c.getClaveBodega(), c.getNombreEstado(), c.getFolioContrato(), 
										c.getPaternoProductor(), c.getMaternoProductor(), c.getNombreProductor(),  c.getCurpProductor(), c.getRfcProductor(), c.getFolioDocPago(), c.getBancoSinaxc());								
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(c.getClaveBodega()!=null ? c.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
...
//									cell.setCellValue(bfp.getImpTotalPagoSinaxc()!=null?bfp.getImpTotalPagoSinaxc()+"":"");
						...
						if(lstFacturasVsPago.size()>0){//En el listado se guarda las facturas menores a lo pagado
							...
							for(FacturasVsPago f: lstFacturasVsPago){
								...
								rDAO.actualizaFacMayBolOPagMenFac(folioCartaAdhesion, f.getClaveBodega(), f.getNombreEstado(), f.getFolioContrato(),
										f.getPaternoProductor(), f.getMaternoProductor(), f.getNombreProductor(), f.getCurpProductor(), f.getRfcProductor(),false, true);								
								row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(f.getClaveBodega()!=null ? f.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
						...
						if(lstPagosCamposRequeridos.size()>0){
							...
							for(PagosCamposRequeridos bo: lstPagosCamposRequeridos){
								...
								bd.setMensaje(bo.getClaveBodega()+";"+bo.getNombreEstado()+";"+(siAplicaFolioContrato?bo.getFolioContrato()+";":"")
										+bo.getPaternoProductor()+";"+bo.getMaternoProductor()+";"+bo.getNombreProductor()+";"
										+bo.getCurpProductor()+";"+bo.getRfcProductor()+";"+bo.getFolioDocPago()+";"
					...
					}else if(l.getIdCriterio() == 28){//"6.4 PRECIO MENOR AL ESTABLECIDO EN AVISO"
						...
						if(lstPrecioPagadoMenorQueAviso.size() > 0){//"6.4 PRECIO MENOR AL ESTABLECIDO EN AVISO" 
							llenarBitacora(true, l.getIdCriterio()); //Guardar en bitacora
							msj = l.getCriterio();
							row = sheet.createRow(countRow);
							cell = row.createCell(countColumn);
							...
							for(PrecioPagadoNoCorrespondeConPagosV f: lstPrecioPagadoMenorQueAviso){
								...
								rDAO.actualizaPagosInconsistente(folioCartaAdhesion, f.getClaveBodega(), f.getNombreEstado(), f.getFolioContrato(),
										f.getPaternoProductor(), f.getMaternoProductor(), f.getNombreProductor(), f.getCurpProductor(), f.getRfcProductor(), true);
							 	row = sheet.createRow(++countRow);
								cell = row.createCell(countColumn);
								cell.setCellValue(f.getClaveBodega()!=null ? f.getClaveBodega()+"":"");
								cell = row.createCell(++countColumn);
					...
					}else if(l.getIdCriterio() == 29){//"9.1 PAGO MENOR DE LA COMPENSACIÓN DE BASES"
						sheet = wb.createSheet("PAG MENOR COMP DE BASES");
						sheet = setMargenSheet(sheet);
						countRow = 0;
						countColumn = 0;
						lstPagMenorCompBases = rDAO.pagoMenorCompBases(folioCartaAdhesion); 
						if(lstPagMenorCompBases.size() > 0){//"9.1 PAGO MENOR DE LA COMPENSACIÓN DE BASES" 
							llenarBitacora(true, l.getIdCriterio()); //Guardar en bitacora
							msj = l.getCriterio();
							row = sheet.createRow(countRow);
							cell = row.createCell(countColumn);
							...
							cell.setCellValue("Importe Factura");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Importe Compensación");
							cell = row.createCell(++countColumn);
							cell.setCellValue("Importe Pagado");
							cell = row.createCell(++countColumn);
							...
							for(PagMenorCompBases f: lstPagMenorCompBases){
								...
								cell.setCellValue(f.getImpSolFacVenta()!=null?f.getImpSolFacVenta()+"":"");
								cell = row.createCell(++countColumn);
								cell.setCellValue(f.getImpCompTonAXC()!=null?f.getImpCompTonAXC()+"":"");
								cell = row.createCell(++countColumn);
								cell.setCellValue(f.getImpTotalPagoSinaxc()!=null?f.getImpTotalPagoSinaxc()+"":"");
								cell = row.createCell(++countColumn);
...
//		lstBitacoraRelComprasPorGrupo = rDAO.consultaBitacoraRelcomprasHCO(folioCartaAdhesion);
	...
	public String getCriteriosByPrograma() {
		return criteriosByPrograma;
	}

	public void setCriteriosByPrograma(String criteriosByPrograma) {
		this.criteriosByPrograma = criteriosByPrograma;
	}

	public List<PagMenorCompBases> getLstPagMenorCompBases() {
		return lstPagMenorCompBases;
	}

	public void setLstPagMenorCompBases(List<PagMenorCompBases> lstPagMenorCompBases) {
		this.lstPagMenorCompBases = lstPagMenorCompBases;
	}

	public List<RelacionComprasTMP> getLstFGlobalVsFIndividual() {
		return lstFGlobalVsFIndividual;
	}

	public void setLstFGlobalVsFIndividual(
