...
public class AutorizacionPagosAction extends ActionSupport implements SessionAware, Serializable {
	...
	public String busquedaAutorizacionPagos(){
		try {			
			...
			estatusCve = 7;
			lstPagosV = pDAO.consultaPagosV(-1, 0, 0, "", "", "7,9", 0, 0,"");
			if(lstPagosV.size()>0){
				setSiHayPagosXAutorizar(true);
			}
	...
	public String realizaBusquedaAutorizacionPago(){
		...
		String fechaFinS = "";
			try{
				...
				if(estatusCve != null){
					if(estatusCve.intValue()==-1){
						estatusCve = null;
					}
				}
				
				/**recupera datos a través de los criterios de busqueda seleccionado por el usuario**/
				lstPagosV = pDAO.consultaPagosV(-1, idPrograma, idComprador, fechaInicioS, fechaFinS, (estatusCve != null ? estatusCve.toString(): null), 0, idEspecialista, null);
				
				if(lstPagosV.size()>0){
					for(PagosV p: lstPagosV){
						if(p.getEstatus()==7 || p.getEstatus() == 9){
							siHayPagosXAutorizar = true;
							break;
						}	
					}		
				}				
		...
		try{
			...
			if(estatusId!=null){
				for(int i=0; i<idPagos.length; i++){
					...
					}else if(estatusId[i].intValue()== 8 || estatusId[i].intValue()== 9){
			...
			for(int i=0; i< idPagosCopia.length; i++){
				if(idPagosCopia[i]!=null){
					if(idPago.intValue() == idPagosCopia[i].intValue()){
						...
						if(pagos.getEstatus()==7 || pagos.getEstatus() == 9){
	...
	public Integer getEstatusCve() {
		return estatusCve;
	}

	public void setEstatusCve(Integer estatusCve) {
	...
	public boolean isSiHayPagosXAutorizar() {
		return siHayPagosXAutorizar;
	}

	public void setSiHayPagosXAutorizar(boolean siHayPagosXAutorizar) {
		this.siHayPagosXAutorizar = siHayPagosXAutorizar;
	}	
}
