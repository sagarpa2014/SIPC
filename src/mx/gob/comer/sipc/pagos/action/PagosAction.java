...
public class PagosAction extends ActionSupport implements SessionAware, Serializable{
	...
	private SolicitudPagoDAO sDAO = new SolicitudPagoDAO();
	...
	private Pagos p, pAux;
	private PagosDetalle pe, peAux;
	private String log;
	private List<ProgramaCompradorV> lstComByPrograma;
	private String ctaBancaria;
	private PagosV pagosV;
	private Long idPago;
	private Long idPagoDev;
	private int editar;
	private int criterioPago;
	private String etapa;
	
	private List<CartaAdhesion> lstCartas; 
	private List<PagosV> lstConsultaPagosCA;
	private List<PagosV> lstConsultaDevolucionesCA;
	private PagosV pv;
	private PagosV dev;
	...
	private Double volumenDev;
	private Double importeDev;
	private File doc;
	private File doc2;
	private String docFileName;
	private String doc2FileName;
	private String ext;
	private String rutaLineaCaptura;
	private String rutaComprobantePago;
	...
	public String busquedaPagosCADevoluciones(){
		try{
			session = ActionContext.getContext().getSession();
			//lstCartas = sDAO.consultaCartaAdhesion(null);
		}catch (JDBCException e) {
			addActionError("Ocurrió un error inesperado, favor de informar al administrador");
			AppLogger.error("errores","Ocurrió un error en busquedaPagosCADevoluciones debido a: " +e.getCause() );
			e.printStackTrace();	    
	    } catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String consultaPagosCADevoluciones(){
		try{
			session = ActionContext.getContext().getSession();
			int idPerfil = (Integer) session.get("idPerfil");
			if(idPerfil == 6){
				idEspecialista = (Integer) session.get("idEspecialista");
				lstConsultaPagosCA = pDAO.consultaPagosV(0, 0, 0, null, null, "5", 0, idEspecialista, cartaAdhesion);
			}else{
				lstConsultaPagosCA = pDAO.consultaPagosV(0, 0, 0, null, null, "5", 0, 0, cartaAdhesion);
			}
			
			//lstCartas = sDAO.consultaCartaAdhesion(null);
		}catch (JDBCException e){
			addActionError("Ocurrió un error inesperado, favor de informar al administrador");
			AppLogger.error("errores","Ocurrió un error en consultaPagosCADevoluciones debido a: " +e.getCause() );
			e.printStackTrace();
		} 
		return SUCCESS;
	}

	public String consultaDevolucionesPagosCA(){
		try{
			session = ActionContext.getContext().getSession();
			lstConsultaDevolucionesCA = pDAO.consultaPagosV(0, 0, 0, null, null, "10", 0, 0, cartaAdhesion);
			pv = pDAO.consultaPagosV(idPago, 0, 0, null, null, "5", 0, 0, cartaAdhesion).get(0);
			cartaAdhesion = pv.getNoCarta();			
		}catch (JDBCException e){
			addActionError("Ocurrió un error inesperado, favor de informar al administrador");
			AppLogger.error("errores","Ocurrió un error en consultaDevolucionesPagosCA debido a: " +e.getCause() );
			e.printStackTrace();
		} 
		return SUCCESS;
	}

	public String capturaDevolucionPagoCA(){
		try{
			if(registrar!=0){
				pv = pDAO.consultaPagosV(idPago, 0, 0, null, null, "5", 0, 0, cartaAdhesion).get(0);
				if (registrar==2){
	...
	public String registroDevolucionPagoCA(){
		try{
			session = ActionContext.getContext().getSession();
			if(registrar == 1){//Nuevo registro
				p = new Pagos();
				pAux = pDAO.consultaPagos(idPago).get(0);
				p.setUsuarioCreacion((Integer) session.get("idUsuario"));
				p.setFechaCreacion(new Date());
				p.setIdPrograma(pAux.getIdPrograma());
				p.setIdComprador(pAux.getIdComprador());
				p.setNoCarta(pAux.getNoCarta());				
			}else if(registrar == 2){ //Edicion de registro
				p = pDAO.consultaPagos(idPagoDev).get(0);
				p.setUsuarioModificacion((Integer) session.get("idUsuario"));
				p.setFechaModificacion(new Date());				
			}
			p.setEstatus(10);
			p.setImporte(importeDev);
			p.setVolumen(volumenDev);
			...
			if(doc!=null){
				...
				if(errorSistema == 1){
					addActionError("El archivo de la Linea de Captura no debe exceder de 3MB, por favor verifique");
					return SUCCESS;
				}
			}
			...
			if(doc2!=null){
				...
				if(errorSistema == 1){
					addActionError("El archivo del Comprobante de Pago no debe exceder de 3MB, por favor verifique");
					return SUCCESS;
				}
			}
			...
			if(docFileName!=null&&docFileName!=""){
				...
				nombreReporte = "LineaCaptura-"+idPago+"-"+new java.text.SimpleDateFormat("yyyyMMddHHmm").format(new Date())+ext; // Archivo Justificacion de Cambios
				rutaSalida = cDAO.consultaParametros("RUTA_DEVOLUCIONES");
				if (!rutaSalida.endsWith(File.separator)){
					rutaSalida += File.separator;
				}							
				Utilerias.cargarArchivo(rutaSalida, nombreReporte, doc);			
				p.setRutaArchivoLC(rutaSalida+nombreReporte);
			}

			if(doc2FileName!=null&&doc2FileName!=""){
				ext = doc2FileName.toLowerCase().substring(doc2FileName.lastIndexOf("."));
				nombreReporte = "ComprobantePago-"+idPago+"-"+new java.text.SimpleDateFormat("yyyyMMddHHmm").format(new Date())+ext; // Archivo Justificacion de Cambios
				rutaSalida = cDAO.consultaParametros("RUTA_DEVOLUCIONES");
				if (!rutaSalida.endsWith(File.separator)){
					rutaSalida += File.separator;
				}							
				Utilerias.cargarArchivo(rutaSalida, nombreReporte, doc2);			
				p.setRutaArchivoPagoLC(rutaSalida+nombreReporte);
			}
						
			if(registrar == 2){ //Edicion de registro
				//borra el detalle pagos asociados al pago
				pDAO.borraDetallePagos(idPagoDev);				
			}			
			p.setPagosDetalle(new HashSet<PagosDetalle>());			
			peAux = pDAO.consultaPagosDetalle(0, idPago).get(0);
			pe = new PagosDetalle();
			pe.setIdEstado(peAux.getIdEstado());
			pe.setImporte(importeDev);
			pe.setVolumen(volumenDev);
			pe.setIdCultivo(peAux.getIdCultivo());
			pe.setIdVariedad(peAux.getIdVariedad());
			pe.setEtapa(peAux.getEtapa());
			pe.setCuota(peAux.getCuota());
			p.getPagosDetalle().add(pe);
			cDAO.guardaObjeto(p);
	...
	public String eliminarDevolucionPagoCA(){
		try{
			session = ActionContext.getContext().getSession();
			p = pDAO.consultaPagos(idPagoDev).get(0);
			cDAO.borrarObjeto(p);				
			...
			pv = pDAO.consultaPagosV(idPago, 0, 0, null, null, "5", 0, 0, cartaAdhesion).get(0);
			cartaAdhesion = pv.getNoCarta();			
			cuadroSatisfactorio = "Se eliminó satisfactoriamente el registro";			
		}catch (JDBCException e) {
	    	e.printStackTrace();
	...
	public List<CartaAdhesion> getLstCartas() {
		return lstCartas;
	}

	public void setLstCartas(List<CartaAdhesion> lstCartas) {
	...
	public void setCartaAdhesion(String cartaAdhesion) {
		this.cartaAdhesion = cartaAdhesion.trim();
	}

	public List<PagosV> getLstConsultaPagosCA() {
		return lstConsultaPagosCA;
	}

	public void setLstConsultaPagosCA(List<PagosV> lstConsultaPagosCA) {
		this.lstConsultaPagosCA = lstConsultaPagosCA;
	}
	...
	public Double getVolumenDev() {
		return volumenDev;
	}

	public void setVolumenDev(Double volumenDev) {
		this.volumenDev = volumenDev;
	}

	public Double getImporteDev() {
		return importeDev;
	}

	public void setImporteDev(Double importeDev) {
		this.importeDev = importeDev;
	}
	...
	public String getRutaLineaCaptura() {
		return rutaLineaCaptura;
	}

	public void setRutaLineaCaptura(String rutaLineaCaptura) {
		this.rutaLineaCaptura = rutaLineaCaptura;
	}
	...
	public void setRutaComprobantePago(String rutaComprobantePago) {
		this.rutaComprobantePago = rutaComprobantePago;
	}
	...
	public Pagos getpAux() {
		return pAux;
	}

	public void setpAux(Pagos pAux) {
		this.pAux = pAux;
	}
	...
	public List<PagosV> getLstConsultaDevolucionesCA() {
		return lstConsultaDevolucionesCA;
	}

	public void setLstConsultaDevolucionesCA(List<PagosV> lstConsultaDevolucionesCA) {
		this.lstConsultaDevolucionesCA = lstConsultaDevolucionesCA;
	}
	...
	public Long getIdPagoDev() {
		return idPagoDev;
	}

	public void setIdPagoDev(Long idPagoDev) {
		this.idPagoDev = idPagoDev;
	}

	public PagosV getDev() {
		return dev;
	}

	public void setDev(PagosV dev) {
		this.dev = dev;
	}

	public PagosDetalle getPe() {
		return pe;
	}

	public void setPe(PagosDetalle pe) {
		this.pe = pe;
	}

	public PagosDetalle getPeAux() {
		return peAux;
	}
