...
public class RepresentanteLegalAction extends ActionSupport implements SessionAware{
	...
	public String registrarRepLegal(){
		try{
			...
			if(estadoCivil == null  || estadoCivil == -1){
				representante.setEstadoCivil(null);
			}else{
				representante.setEstadoCivil(estadoCivil);
			}
			
			if(nacionalidad == null || nacionalidad.isEmpty()){
				representante.setNacionalidad(null);
			}else{
				if (nacionalidad.equals("1")) {
					representante.setNacionalidad(1);
				} else {
					representante.setNacionalidad(2);
				}
			}

			if(tipoIdentificacion == null  || tipoIdentificacion == -1){
				representante.setTipoIdentificacion(null);
			}else{
				representante.setTipoIdentificacion(tipoIdentificacion);
			}
			
			if(folioIdentificacion == null || folioIdentificacion.isEmpty()){
				representante.setFolioIdentificacion(null);
			}else{
				representante.setFolioIdentificacion(folioIdentificacion.toUpperCase());
			}

			if(rfe == null || rfe.isEmpty()){
				representante.setRfe(null);
			}else{
				representante.setRfe(rfe.toUpperCase());
			}
			...
			if(numInterior == null || numInterior.isEmpty()){
				...
				representante.setNumInterior(numInterior.toUpperCase());
			}
			...
			if(documentoAcredita == null || documentoAcredita.isEmpty()){
				representante.setDocumentoAcredita(null);
			}else{
				representante.setDocumentoAcredita(documentoAcredita.toUpperCase());
			}

			if(vigenciaPoder == null){
				representante.setVigenciaPoder(null);
			}else{
				representante.setVigenciaPoder(vigenciaPoder);
			}
	...
	public String recuperaDatosRepresentante(){
		try{
			...
			fax = representanteLegalV.getFax();
			correoElectronico = representanteLegalV.getCorreoElectronico();
			colonia = representanteLegalV.getColonia();
			numExterior = representanteLegalV.getNumExterior();
			numInterior = representanteLegalV.getNumInterior();			
			calle = representanteLegalV.getCalle();
			codigoPostal = representanteLegalV.getCodigoPostal();
			
			estadoCivil = representanteLegalV.getEstadoCivil();
			if(representanteLegalV.getNacionalidad()!=null){
				nacionalidad = representanteLegalV.getNacionalidad().toString();
			}
			tipoIdentificacion = representanteLegalV.getTipoIdentificacion();
			folioIdentificacion = representanteLegalV.getFolioIdentificacion();
			rfe = representanteLegalV.getRfe();
			documentoAcredita = representanteLegalV.getDocumentoAcredita();
			vigenciaPoder = representanteLegalV.getVigenciaPoder();
