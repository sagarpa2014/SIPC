...
public class AtentaNotaPagos extends PdfPageEventHelper {
	...
	private List<PagosDetalleGroupV> pagoDet;
	...
	public void generarAtentaNotaPago(Long idPago) throws Exception{	
		...
		if(idPago != -1){
			...
			pagoDet = pDAO.consultaPagosDetalleVByCultVariedadEdo(idPago);
	...
	private void getAnexoDetallePagos() throws DocumentException {
		...
		for (PagosDetalleGroupV pagoAux : pagoDet){
			if(apa.getCriterioPago()== 1){
				if(pagoDet.get(0).getVariedad()==null||pagoDet.get(0).getVariedad()==""){
...
/*		parrafo =  new Paragraph("", TIMESROMAN10NORMAL);
	...
	private void getTitulo() throws DocumentException {
		...
		.append("REPORTE DE VOLUMEN ACREDITADO PARA INCENTIVO");
	...
	private void getCuerpo() throws DocumentException{
		...
		textoEsquema.append(new Chunk("En referencia al ",TIMESROMAN10NORMAL)).append(new Chunk(pagoV.get(0).getNombrePgrLarga(),TIMESROMAN10));
		...
		texto.append(new Chunk("Derivado de la revisión del expediente con Carta de Adhesión ",TIMESROMAN10NORMAL))
			 ...
			 .append(new Chunk(", se ha identificado un ",TIMESROMAN10NORMAL)).append(new Chunk(leyendaCriterio,TIMESROMAN10)).append(new Chunk(" susceptible de ser apoyado correspondiente al ",TIMESROMAN10NORMAL))
...
//		document.add(parrafo);
    ...
    private void getEmisor() throws DocumentException {
		...
		//PdfPTable t = new PdfPTable(x);
		parrafo =  new Paragraph(new Chunk("Elabora:\n\n\n\n\n", TIMESROMAN10));
		//parrafo.add(parrafo);
		cell =createCell(parrafo, 0, 1, 1);
		t.addCell(cell);
		parrafo =  new Paragraph(new Chunk("", TIMESROMAN10));
		//parrafo.add(parrafo);
		cell =createCell(parrafo, 0, 1, 1);
		t.addCell(cell);
		parrafo =  new Paragraph(new Chunk("Vo. Bo.\n\n\n\n\n", TIMESROMAN10));
		cell =createCell(parrafo, 0, 1, 1);
		t.addCell(cell);		
		parrafo = new Paragraph(apa.getEmisor().getNombre()+"\n"+apa.getEmisor().getPuesto().toUpperCase(), TIMESROMAN10);
		...
		cell =createCell(parrafo, 0, 1, 1);
		t.addCell(cell);
		parrafo = new Paragraph(apa.getVoBo().getNombre().toUpperCase()+" "+(apa.getVoBo().getPaterno()!=null?apa.getVoBo().getPaterno().toUpperCase()+" ":"")+
				(apa.getVoBo().getMaterno()!=null?apa.getVoBo().getMaterno().toUpperCase()+" ":"")
		+"\n"+apa.getVoBo().getPuesto().toUpperCase(), TIMESROMAN10);
		cell =createCell(parrafo, 0, 1, 10);
		t.addCell(cell);		
