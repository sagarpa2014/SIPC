...
<s:form action="realizaBusquedaAutorizacionPago" onsubmit="return chkCamposBusquedaAutorizacionPagos();">
			...
			<fieldset>
				...
				<table>					
						<tr>
							...
							<th>Volumen (ton.)</th>
							<th>Importe Neto ($)</th>
							<s:if test="%{siHayPagosXAutorizar == true}">
								<th>Estatus Nuevo</th>
							</s:if>
							<s:else>
								<th>Descarga</th>
						...
						<s:iterator value="lstPagosV" var="resultado"  status="itStatus">
							...
							<tr>
								...
								<s:if test="%{estatus == 7}">
									<td><s:select id="estatusId%{#itStatus.count}" name="estatusId" onchange="selectEstatus(this.value, %{#itStatus.count});" headerKey="-1" headerValue="-- Pendiente --"	list="#{'9':'REGISTRADO','1':'AUTORIZADO', '8':'RECHAZADO'}"  value="" /></td>
									<td id="contenedorFile<s:property value="%{#itStatus.count}"/>"></td>
								</s:if>	
								<s:elseif test="%{estatus == 9}">
