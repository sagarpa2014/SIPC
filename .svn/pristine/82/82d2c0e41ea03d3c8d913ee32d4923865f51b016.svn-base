		...
		<table class="clean">
			...
			<s:iterator value="lstAuditorSolPago" var="resultado"  status="itStatus">
				<tr>
					...
					<s:else>
						<td>
							<s:textfield id="capNumeroAuditor%{#itStatus.count}" name="capNumeroAuditor" disabled="true" maxlength="15" size="20" value="%{numeroAuditor}" onchange="validarNumeroAuditor(this.value, %{#itStatus.count});"/>			
						</td>
						<td id="validarNumeroAuditor<s:property value="%{#itStatus.count}"/>">
							<s:select id="auditorV%{#itStatus.count}" name="selectedAuditorV" list="lstAuditoresExternos" listKey="numeroAuditor" listValue="%{nombre}" headerKey="-1" headerValue="-- Seleccione una opción --" value="%{numeroAuditor}" onchange="" disabled="true"/>
						</td>
						<td>
							<s:textfield id="v%{#itStatus.count}" name="capVolumen" disabled="true" maxlength="15" size="20"  cssClass="cantidad" value="%{volumenAuditor}" />
						</td>					
					</s:else>					
				</tr>	
			</s:iterator>
