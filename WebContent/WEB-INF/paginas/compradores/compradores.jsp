...
<div class="borderBottom"><h1>COMPRADORES</h1></div><br>
	<fieldset>
		...
		<fieldset>
			...
			<display:table id="r"  name="lstCompradores"  list="lstCompradores"  pagesize="50" sort="list" requestURI="/catalogos/ejecutaBusquedaCompradores"  class="displaytag">
				...
				<display:column title="Ver Detalle"  headerClass="sortable" >
					<a href='<s:url value="/catalogos/detallesComprador?idComprador=%{#attr.r.idComprador}&tipoPersona=%{#attr.r.tipoPersona}"/>' class="botonVerDetalles" title="" target="winload" onclick="window.open(this.href, this.target, 'width=800,height=800,scrollbars=yes'); return false;"></a>
			 	</display:column>
			 	...
			 	<s:elseif test="#session.idPerfil==12">
			 		<display:column title="Editar"  headerClass="sortable" >
			 			<s:if test="#attr.r.compradorTemporal == 1">
			 				<a href='<s:url value="/catalogos/recuperaDatosComprador?editar=3&idComprador=%{#attr.r.idComprador}"/>' class="editar" title="" ></a>
			 			</s:if>
			 		</display:column>
			 	</s:elseif>
