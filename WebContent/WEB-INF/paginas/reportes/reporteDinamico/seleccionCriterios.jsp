...
<table>
	...
	<s:iterator value="lstOpciones" var="resultado"  status="itStatus">
		<tr>
			...
			<s:elseif test="%{tipoReporte==0}">
				<s:if test="%{id==6}">
					<td class="td1"><s:select id="idCultivo" name="idCultivo" list="lstCultivos" listKey="idCultivo" listValue="%{cultivo}" headerKey="-1" headerValue="-- Seleccione una opción --" cssClass="elementoOculto" onchange="recuperaVariedadByCultivo(this.value);"/></td>
				</s:if>
				<s:elseif test="%{id==7}">
					<td id="variedad" class="td1"><s:select id="idVariedad" name="idVariedad" list="lstVariedad" listKey="idVariedad" listValue="%{variedad}" headerKey="-1" headerValue="-- Seleccione una opción --" cssClass="elementoOculto" /></td>
				</s:elseif>
				<s:elseif test="%{id==8}">
					<td class="td1"><s:select id="idBodega" name="idBodega" list="lstBodega" listKey="idBodega" listValue="%{claveBodega}" headerKey="-1" headerValue="-- Seleccione una opción --" cssClass="elementoOculto"/></td>
				</s:elseif>				
			</s:elseif>
			<s:elseif test="%{tipoReporte==1}">
				<s:if test="%{id==5}">
				...
				<s:elseif test="%{id==6}">
					<td class="td1"><s:select id="idVariedad" name="idVariedad" list="lstVariedad" listKey="idVariedad" listValue="%{variedad}" headerKey="-1" headerValue="-- Seleccione una opción --" cssClass="elementoOculto" /></td>
				</s:elseif>
				<s:elseif test="%{id==7}">
					<td class="td1"><s:select id="idBodega" name="idBodega" list="lstBodega" listKey="idBodega" listValue="%{claveBodega}" headerKey="-1" headerValue="-- Seleccione una opción --" cssClass="elementoOculto"/></td>
				</s:elseif>								
