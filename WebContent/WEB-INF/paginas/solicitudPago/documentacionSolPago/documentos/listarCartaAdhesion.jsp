			...
			<s:if test='#session.idPerfil==10 || #session.idPerfil==1'>	
				...
				<display:column title="Cierre Expediente" class="dcolor">
					<s:if test='#attr.r.estatusCA==9'>
							<a href='<s:url value="/solicitudPago/cierreExpedienteCARevocar?folioCartaAdhesion=%{#attr.r.folioCartaAdhesion}&idPrograma=%{idPrograma}"/>' title="" onclick="if (confirm('¿Esta seguro de Reabrir Expediente de la Carta de Adhesión?')){}else{return false;}">Reabrir</a>										
					</s:if>
					<s:elseif test='#attr.r.estatusCA==4 || #attr.r.estatusCA==5 '>
						<a href='<s:url value="/solicitudPago/cierreExpedienteCA?folioCartaAdhesion=%{#attr.r.folioCartaAdhesion}&idPrograma=%{idPrograma}"/>' title="" onclick="if (confirm('¿Esta seguro de Cerrar Expediente de la Carta de Adhesión?')){}else{return false;}">Confirmar</a>
					</s:elseif>
				</display:column>													
