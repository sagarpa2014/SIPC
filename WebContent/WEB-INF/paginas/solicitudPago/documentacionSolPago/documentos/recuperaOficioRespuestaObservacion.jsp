...
<s:if test="estatusCA == 4 && sustituirArchivo != true" >
	<div class="clear"></div>
...
<s:if test="lstOficioRespuestaSP.size()>0">
	...
	<table class="clean">
		<s:iterator value="lstOficioRespuestaSP" var="resultado"  status="itStatus">
			<tr>
				<td><label class="left1"><span class="norequerido">*</span>Oficio de Respuesta</label></td>
				<td>
					<s:if test="sustituirArchivo == true">
						<s:file name="docRespA"  id="resp%{#itStatus.count}" onchange="llenarArrayOficioResp(%{idOficioRespuesta},  %{#itStatus.count});" />
						<s:hidden id="idOficioResp%{#itStatus.count}" name="idOficioRespuestaA" value="%{}"/>
					</s:if>
					<s:else>
						<a href="<s:url value="/devuelveArchivoByRuta?rutaCompleta=%{rutaDocRespuesta+nomArchivoRespuesta}"/>" title="Archivo Oficio Observaci&oacute;n">
							<s:text name="fecha"><s:param value="%{fechaDocResp}"/></s:text>
						</a>
					</s:else>
				</td>
			</tr>
