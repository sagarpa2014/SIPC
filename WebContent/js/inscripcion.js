...
function recuperaCultivoByVariedad(idCultivo, count){
	...
	if(volxCulVar==1){
		...
		var i = 0, cultivoVXCV ="", j = 0, variedadVXCV ="", elementoRepetido=0;
		var selectedVariedad = new Array();
		var textSelect ="";
		var cadenaOption = '<option value="-1">-- Seleccione una opción --</option>';
		for(i=1;i<=parseInt(numCamposVXCV);i++){
			cultivoVXCV =  $('#cultivoVXCV'+i).val();
			if(idCultivo == cultivoVXCV){
				variedadVXCV = $('#variedadVXCV'+i).val();
				for(j = 0; j < selectedVariedad.length; j++ ){
  					if(selectedVariedad[j]==variedadVXCV){
  						elementoRepetido = 1;
  						break;
  					}  
  				}
				if(elementoRepetido != 1){
					selectedVariedad.push(variedadVXCV);
  		  			cadenaOption = cadenaOption+'<option value="'+variedadVXCV+'">'+$('#variedadVXCV'+i+' option:selected').html()+'</option>';
  				}
  				elementoRepetido = 0;
...
function recuperaCultivoByVariedadVXCV(idCultivo, count){
	...
	if(numCampos > 0){
  		for (i=1;i<=parseInt(numCamposVXCV);i++){
  			cultivoVXCV =  $('#cultivoVXCV'+i).val();
  			if(cultivoVXCV != -1){
  				//Verifica que el elemento no se encuentre en el arreglo
  				for(j = 0; j < selectedCultivo.length; j++ ){
  					if(selectedCultivo[j]==cultivoVXCV){
  						elementoRepetido = 1;
  						break;
  					}
  				}
  				if(elementoRepetido != 1){
  					selectedCultivo.push(cultivoVXCV);
  		  			cadenaOption = cadenaOption+'<option value="'+cultivoVXCV+'">'+$('#cultivoVXCV'+i+' option:selected').html()+'</option>';
  				}
  				elementoRepetido = 0;
  			}
  		}
  		var siPermaneceCultivo = 0;
  		var textSelect = "";
  		for(i=1; i<=parseInt(numCampos); i++){
  			//Verifica que el elemento seleccionado se encuentre en la lista de cultivos en la 
  			cul = $('#c'+i).val();
  			for(j=0; j< selectedCultivo.length; j++){
  				if(cul == selectedCultivo[j]){
  					siPermaneceCultivo = 1;
  					break;
  				}
  			}  			
...
function chkCamposACA(){
	...
	if(registrar==0){
		...
		for (i=1;i<parseInt(numCampos)+1;i++){
			...
			if(idCriterioPago == 1 || idCriterioPago == 3){
				...
				importe = $('#i'+i).val();
				if(cult==-1 || edo==-1  || variedad== -1 || (claveBodega == "" || claveBodega == null)  || (importe == "" || importe == null)){
		...
		totalVolumen = totalVolumen.toFixed(3);
		
		if(idCriterioPago == 1 || idCriterioPago==3){			
			var volumenDisponibleValidar = $('#volumenDisponibleValidar').val();
			volumenDisponibleValidar =  parseFloat(volumenDisponibleValidar);
			volumenDisponibleValidar = volumenDisponibleValidar.toFixed(3);
			
			if(parseFloat(totalVolumen) > volumenDisponibleValidar){
				$('#dialogo_1').html('El volumen total asignado '+totalVolumen+' no puede exceder del volumen disponible'+volumenDisponibleValidar+', favor de verificar');
			   		abrirDialogo();
			 		return false;
			}
			...
			importeDisponibleValidar = importeDisponibleValidar.toFixed(2);
			if(parseFloat(totalImporte) > importeDisponibleValidar){
			...
			importeInscripcionValidar = parseFloat(importeInscripcionValidar);
			importeInscripcionValidar = importeInscripcionValidar.toFixed(2);
			if(parseFloat(totalImporte) > importeInscripcionValidar){
		...
		//Habilitando a capCuota y capImporte para que sean registrados en la tabla de asignacion_cartas_adhesion
		$("input[name^='capCuota']").removeAttr('disabled');
		$("input[name^='capImporte']").removeAttr('disabled');
	...
	if($('#complementoPorampliacionChk').length > 0){
			...
			if(idCriterioPago == 1 || idCriterioPago == 3){
				...
				if(!volumen.match(patron)){
					...
					if(volumenXCultivoVariedad==1){
						console.log("Valida el volumen maximo por cultio variedad");
						//Valida el volumen maximo por cultivo variedad
						volumenMaximoCultivoVariedad = parseFloat(volumenMaximoCultivoVariedad);
						volumenMaximoCultivoVariedad = volumenMaximoCultivoVariedad.toFixed(3);
						if(parseFloat(volumen) > volumenMaximoCultivoVariedad){
							$('#dialogo_1').html('El valor del volumen no debe rebasar el máximo por cultivo variedad en el registro '+i);
							abrirDialogo();
							return false;
						}
				...
				importe = $('#i'+i).val();
				if(cult==-1 || edo==-1  || (importe == "" || importe == null) || variedad==-1 || (claveBodega == "" || claveBodega == null) ){
		   			$('#dialogo_1').html('Capture los valores del registro '+i+' en los estados a apoyar');
			   		abrirDialogo();
			 		return false;
				}
		...
		if(idCriterioPago == 1 || idCriterioPago==3){
			...
			var importeDisponibleValidar = $('#importeDisponibleValidar').val();
			importeDisponibleValidar = parseFloat(importeDisponibleValidar);
			importeDisponibleValidar = importeDisponibleValidar.toFixed(3);
			if(parseFloat(totalImporte) > importeDisponibleValidar){
				$('#dialogo_1').html('El importe total asignado '+totalImporte+' no puede exceder del importe disponible, favor de verificar');
			   		abrirDialogo();
			 		return false;
	...
	//Para ambos registrar == 0 y complementoPorampliacionChk se verifica si no se capturo una bodega incorrecta
	if(claveBodegaInvalida == 1){
		$('#dialogo_1').html('Existe una clave de bodega incorrecta, por favor verifique');
   		abrirDialogo();
 		return false;
	}
	...
	//if(volumenXCultivoVariedad == 1){
		...
		if((idCultivo != -1) && (idVariedad != -1) && (idEstado != -1) &&(v!="" && v !=null)){
			...
			$.ajax({
				   async: false,
				   type: "POST",
				   url: "recuperarDatosCuotaImpCulXVariedad",
				   data: "idCultivo="+idCultivo+
...
function validaClaveBodegaSI(claveBodega, count){
	...
	$.ajax({
		   async: false,
		   type: "POST",
		   url: "validarClaveBodega", 
		   data: "claveBodega="+claveBodega+
	   			"&count="+count,
   			success: function(data){
				   var $response=$(data);
				   var error = $response.filter('#errorClaveBodega'+count).text();
				   $('#claveBodegaInvalida').val(error);
...
function agregarNumCamposVXCV(){	
	var numCamposVXCV = $('#numCamposVXCV').val();
	var editar = $('#editar').val();
	numCamposVXCV = parseInt(numCamposVXCV) + 1;
	$('#numCamposVXCV').val(numCamposVXCV);
	$("#removerNumCamposVXCV").fadeIn('slow');
	...
	}else if(numCamposVXCV !=1 && numCamposVXCV > 0){
		var tds = '<tr>';
		tds += '<td>'+numCamposVXCV+'</td>';
		tds += '<td id="contenedorCultivoVXCV'+numCamposVXCV+'"></td>';
		tds += '<td id="contenedorVariedadVXCV'+numCamposVXCV+'"><select id="variedadVXCV'+numCamposVXCV+'"><option value="-1">-- Seleccione una opción --</option></select></td>';
		tds += '<td><input type="text" id="volumenVXCV'+numCamposVXCV+'" name="selectedVolumenVXCV" maxlength="15" size="20" class="cantidad" ></td>';
		tds += '</tr>';
		$("#tableVolumenPorCultivoVariedad").append(tds);
		$('#cultivoVXCV1').clone().attr('id', 'cultivoVXCV'+numCamposVXCV).attr('onchange', 'recuperaCultivoByVariedadVXCV(this.value,'+numCamposVXCV+')').appendTo('#contenedorCultivoVXCV'+numCamposVXCV);
		//$('#variedadVXCV1').clone().attr('id', 'variedadVXCV'+numCamposVXCV).appendTo('#contenedorVariedadVXCV'+numCamposVXCV);
		$('#cultivoVXCV'+numCamposVXCV).val(-1);
		$('#variedadVXCV'+numCamposVXCV).val(-1);
	}	
}

function removerNumCamposVXCV(){	
	var numCamposVXCV = $('#numCamposVXCV').val();
	if(numCamposVXCV > 0){
		if($('#tableVolumenPorCultivoVariedad').length){
			$('#tableVolumenPorCultivoVariedad').each(function(){
	...
	} //numCamposAccionesR > 0
	if(numCamposVXCV == 0){
		$("#removerNumCamposVXCV").fadeOut('slow');
	}

}

function agregarNumCamposCultivoEstadoIniProg(){		
	...
	if(idCriterioPago == 1 || idCriterioPago ==3){
		...
		if(volxCulVar == 0){
			var numCamposVXCV = $('#numCamposVXCV').val();
			if(numCamposVXCV == 0){
				$('#dialogo_2').html('Capture los registros para el volumen por cultivo variedad');
		   		abrirDialogo2();
				return false;
			}else{
				var i =0;
	...
	$('#numCampos').val(numCampos);
	$("#removerNumCamposCultivoEstadoIniProg").fadeIn('slow');
	...
	if(numCampos == 1){		
	...
	}else if(numCampos !=1 && numCampos > 0){
		var tds = '<tr>';
		tds += '<td>'+numCampos+'</td>';
		...
		$('#e1').clone().attr('id', 'e'+numCampos).appendTo('#contenedorListEdo'+numCampos);
...
function removerNumCamposCultivoEstadoIniProg(){	
	var numCampos = $('#numCampos').val();
	if(numCampos > 0){
		if($('#tableCultivoEstadoIniProg').length){
			$('#tableCultivoEstadoIniProg').each(function(){
			    if($('tbody', this).length > 0){
			        $('tbody tr:last', this).remove();
			    }else {
			        $('tr:last', this).remove();
	...
	if(numCampos == 0){
		$("#removerNumCamposCultivoEstadoIniProg").fadeOut('slow');
	}
