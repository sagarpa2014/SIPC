...
function chkCamposDocumentacion(){	
	...
	if(alcanceDocumentacion=="true"){
		...
		if(checados == 0){
			$('#dialogo_1').html('Si seleccionó "Alcance al oficio de Observaciones" debe marcar al menos un documento observado');
			abrirDialogo();
			return false;
		}
...
// CAPTURA DE PERIODOS DE DICTAMEN DEL AUDITOR - FIN
	...
	if(estatusCA==4 && alcanceDocumentacion=="false" && sustituirArchivo=="false"){
	...
	var anio =0;
	/****DOCUMENTO FINIQUITO****/
	var docFin = $('#docFin').val();
	var fechaDocFin = $('#fechaDocFin').val();
	var fechaAcuseFin = $('#fechaAcuseFin').val();
	if(!(docFin == null || docFin=="") || 
	   !(fechaDocFin == null || fechaDocFin=="") || 
	   !(fechaAcuseFin == null || fechaAcuseFin=="")){
		if(docFin == null || docFin==""){
			$('#dialogo_1').html('Seleccione el archivo del finiquito');
			abrirDialogo();
			return false;
		}
		/*****FECHA DOCUMENTO FINIQUITO****/
		var fechaDocFinTemp = 0;
		if(fechaDocFin == null || fechaDocFin==""){
			$('#dialogo_1').html('Seleccione la fecha del documento del finiquito');
			abrirDialogo();
			return false;
		}else{
			dia = fechaDocFin.substring(0,2);
			mes = fechaDocFin.substring(3,5);
			anio = fechaDocFin.substring(6,10);
			fechaDocFinTemp = anio+""+""+mes+""+dia;
		}
		/*****FECHA ACUSE FINIQUITO****/
		var fechaAcuseFinTemp = 0;
		if(fechaAcuseFin == null || fechaAcuseFin==""){
			$('#dialogo_1').html('Seleccione la fecha del acuse del finiquito');
			abrirDialogo();
			return false;
		}else{
			dia = fechaAcuseFin.substring(0,2);
			mes = fechaAcuseFin.substring(3,5);
			anio = fechaAcuseFin.substring(6,10);
			fechaAcuseFinTemp = anio+""+""+mes+""+dia;
			if(parseInt(fechaAcuseFinTemp) < fechaDocFinTemp){
				$('#dialogo_1').html('La fecha del acuse del finiquito no debe ser menor a la fecha del documento');
				abrirDialogo();
				return false;
			}
		}			
...
function chkCamposFianza(){
	...
	if($('#doc32').length){
		...
		docRequerido = $('#docRequerido32').val();
		expediente = recuperaNombreDocumento(idExpediente);
//alert("docRequerido: "+docRequerido+ " expediente 32: "+expediente);
		if(docRequerido=="true"){
			if(doc == null || doc == ""){
				$('#dialogo_1').html('Seleccione el archivo de '+expediente);
				abrirDialogo();
				return false;
	...
	if($('#doc4').length){
		...
		docRequerido = $('#docRequerido4').val();
		expediente = recuperaNombreDocumento(idExpediente);
//alert("docRequerido: "+docRequerido+ " expediente 4: "+expediente);
		if(docRequerido=="true"){
			if(doc == null || doc == ""){
				$('#dialogo_1').html('Seleccione el archivo de '+expediente);
				abrirDialogo();
	...
	if($('#doc5').length){
		...
		docRequerido = $('#docRequerido5').val();
		expediente = recuperaNombreDocumento(idExpediente);
//alert("docRequerido: "+docRequerido+ " expediente 5: "+expediente);
		if(docRequerido=="true"){
			if(doc == null || doc == ""){
				$('#dialogo_1').html('Seleccione el archivo de '+expediente);
				abrirDialogo();
	...
	if($('#doc15').length){
		...
		docRequerido = $('#docRequerido15').val();
		expediente = recuperaNombreDocumento(idExpediente);
//alert("docRequerido: "+docRequerido+ " expediente 15: "+expediente);
		if(docRequerido=="true"){
			if(doc == null || doc == ""){
				$('#dialogo_1').html('Seleccione el archivo de '+expediente);
				abrirDialogo();
	...
	if($('#doc33').length){
		...
		docRequerido = $('#docRequerido33').val();
		expediente = recuperaNombreDocumento(idExpediente);
//alert("docRequerido: "+docRequerido+ " expediente 33: "+expediente);
		if(docRequerido=="true"){
			if(doc == null || doc == ""){
				$('#dialogo_1').html('Seleccione el archivo de '+expediente);
				abrirDialogo();
	...
	if($('#doc14').length){
		...
		docRequerido = $('#docRequerido14').val();
		expediente = recuperaNombreDocumento(idExpediente);
//alert("docRequerido: "+docRequerido+ " expediente 14: "+expediente);
		if(docRequerido=="true"){
			if(doc == null || doc == ""){
				$('#dialogo_1').html('Seleccione el archivo de '+expediente);
				abrirDialogo();
...
function chkCamposPagoCartaAdhesion(){
	...
	if(idPerfil == 6){
		...
		if(solicitudesAtendidas =="" || solicitudesAtendidas == null || solicitudesAtendidas == 0){
			$('#dialogo_1').html('Capture el número de solicitudes atendidas');
	   		abrirDialogo();
			return false;
		}else{
			patron = /^\d{1,5}$/;	
			if(!solicitudesAtendidas.match(patron)){
				$('#dialogo_1').html('El número de solicitudes atendidas es incorrecto, se deben capturar hasta 5 digitos');
		   		abrirDialogo();
				return false;
			}
			
		}	
		if(productoresBeneficiados =="" || productoresBeneficiados==null || productoresBeneficiados ==0 ){ 
			$('#dialogo_1').html('Capture el número de productores beneficiados');
	   		abrirDialogo();
			return false;
		}else{
			patron = /^\d{1,5}$/;	
			if(!productoresBeneficiados.match(patron)){
				$('#dialogo_1').html('El número de productores beneficiados es incorrecto, se deben capturar hasta 5 digitos');
		   		abrirDialogo();
				return false;
			}
		}
		...
		if (criterioPago==1){
			...
			if (volumenApoyar != null && volumenApoyar != ""){
				band = true;
	  			...
	  			totalVolumen = (totalVolumen+parseFloat(volumenApoyar));	  			 
			}
			...
			if ((importeApoyar != null && importeApoyar != "") || (etapa != null && etapa!=-1)){
				...
				totalImporte  = (totalImporte+parseFloat(importeApoyar));	
...
function llenarArrayOficioResp(idOficioResp, count){	
	$('#idOficioResp'+count).val(idOficioResp);	
}

function llenarArrayOficioObs(idOficioObsSolPago, count){	
	$('#idOficioObs'+count).val(idOficioObsSolPago);	
}
...
function chkCamposDevolucion(){	
	...
	if(lineaCaptura == null || lineaCaptura==""){
		$('#dialogo_1').html('Capture la Linea de Captura');
		abrirDialogo();
		return false;
	}

	/*****FECHA EMISION LINEA CAPTURA****/
	var fechaEmision = $('#fechaEmision').val();
	var fechaVigencia = $('#fechaVigencia').val();
	var fechaEmisionTemp = 0;
	if(fechaEmision == null || fechaEmision==""){
		$('#dialogo_1').html('Seleccione la Fecha de Emision de la Linea de Captura');
		abrirDialogo();
		return false;
	}else{
		dia = fechaEmision.substring(0,2);
	...
	var fechaVigenciaTemp = 0;
	if(fechaVigencia == null || fechaVigencia==""){
		$('#dialogo_1').html('Seleccione la Fecha de Vigencia de la Linea de Captura');
		abrirDialogo();
		return false;
	}else{
		dia = fechaVigencia.substring(0,2);
		mes = fechaVigencia.substring(3,5);
		anio = fechaVigencia.substring(6,10);
		fechaVigenciaTemp = anio+""+""+mes+""+dia;
		if(parseInt(fechaVigenciaTemp) < fechaEmisionTemp){
			$('#dialogo_1').html('La Fecha de Vigencia debe ser mayor a la Fecha de Emision de la Linea de Captura');
			abrirDialogo();
			return false;
		}
	}			
	...
	/*****VOLUMEN DEVOLUCION****/
	var volumenDev = $('#volumenDev').val();
	if(volumenDev != null && volumenDev != ''){
		validaVolumen = validarVolumenNeg(volumenDev,0,0);
		if(validaVolumen == 0){
			$('#dialogo_1').html('El valor del Volumen es inválido, se deben capturar valor negativo con decimales y aceptan hasta 10 digitos a la izquierda y 3 máximo a la derecha');
			abrirDialogo();
			return false;
		} else {
			var volumen = $('#volumen').val();
			if(parseFloat(volumenDev) > parseFloat(volumen)){
				$('#dialogo_1').html('El Volumen de la Devolucion no debe ser mayor al Volumen Apoyado');
				abrirDialogo();
				return false;
			}			
		}
	...
	/*****IMPORTE DEVOLUCION****/
	var importeDev = $('#importeDev').val();
	var importe = $('#importe').val();
	if(importeDev == null || importeDev==""){
		$('#dialogo_1').html('Capture el Importe de la Devolucion');
		abrirDialogo();
		return false;
	}else{
		validaImporte = validarImporteNeg(importeDev,0,0);
		if(validaImporte == 0){
			$('#dialogo_1').html('El valor del Importe es inválido, se deben capturar valor negativo con decimales y aceptan hasta 10 digitos a la izquierda y 3 máximo a la derecha');
			abrirDialogo();
			return false;
		} else {
			//alert('importeDev: '+importeDev+' importe: '+importe);
			if(parseFloat(importeDev) > parseFloat(importe)){
				$('#dialogo_1').html('El Importe de la Devolucion no debe ser mayor al Importe Apoyado');
				abrirDialogo();
				return false;
			}			
		}
	...
	var registrar = $('#registrar').val();
	if(registrar==1){
		/*********ARCHIVO LINEA DE CAPTURA*****/
		var doc = $('#doc').val();
		if(doc == null || doc ==""){
			$('#dialogo_1').html('Seleccione el archivo de la Linea de Captura');
			abrirDialogo();
			return false;
		}
		...
		var doc2 = $('#doc2').val();
		if(doc2 == null || doc2 ==""){
			$('#dialogo_1').html('Seleccione el archivo del Comprobante de Pago');
			abrirDialogo();
			return false;
		}
	}

}// END chkCamposDevolucion
...
function validarVolumenNeg(volumen, id, msj){
//	alert('volumen: '+volumen+ 'id: '+id+ 'msj: '+msj);
	var patron =/^-\d{1,10}((\.\d{1,3})|(\.))?$/;
	if (!volumen.match(patron)){	
		if(id!=0){
...
function validarImporteNeg(importe, id, msj){
	var patron =/^-\d{1,10}((\.\d{1,2})|(\.))?$/;
	if (!importe.match(patron)){
		if(id!=0){
			$('#'+id).val(null);	
...
function habilitaPorcentaje(){
	var i = 0;
	var numRegistroDetallePago = $('#numRegistroDetallePago').val();
	numRegistroDetallePago = parseInt(numRegistroDetallePago);
	if(($('#tieneFianza')).is(":checked")) {
		$(".izquierdaOculta").css("display", "inline");
		for(i=1; i<=numRegistroDetallePago; i++){
			$('#volumenApoyar'+i).attr('disabled','disabled');	
		}
	}else{
		$(".izquierdaOculta").css("display", "none");
		for(i=1; i<=numRegistroDetallePago; i++){
			$('#volumenApoyar'+i).removeAttr('disabled');	
		}
	}
